...
export class News extends Component {
    ...
    async componentDidMount() {
        ...
        let url = "https://gnews.io/api/v4/search?q=example&apikey=eba93223e002ccd7034540fc52e2fdaa";
        let data = await fetch(url);
        let filterData = await data.json();
        console.log('filterData ::::',filterData);
        if (filterData && filterData.articles && filterData.articles.length) {
            this.setState({ article: filterData.articles, totalResults: filterData.totalArticles });
        }
    }
    privious = async () => {
        console.log('privious');
        let url = `https://gnews.io/api/v4/search?q=example&apikey=eba93223e002ccd7034540fc52e2fdaa&page=${this.state.page - 1}`;
        let data = await fetch(url);
        let filterData = await data.json();
        if (filterData && filterData.articles && filterData.articles.length) {
                    ...
                    <div className='row '>
                        ...
                        {this.state.article && this.state.article.length && this.state.article.map((e) => {
                            return <div className='col-md-4 ' key={e.url} >
                                <NewsItem title={e.title ? e.title.slice(0, 40) : ""} discription={e.description ? e.description.slice(0, 80) : ""} imgUrl={e.image} newsUrl={e.url} />
                            </div>
                        })}
                    </div>
                    <div className="container d-flex justify-content-between my-5">
                        <button disabled={this.state.page <= 1} className='btn btn-dark' onClick={this.privious}>&larr; Previous </button>
                        <button disabled={this.state.page + 1 > Math.ceil(this.state.totalArticles / 10)} className='btn btn-dark' onClick={this.next}>Next &rarr; </button>
                    </div>
                </div>
            </>
        )
